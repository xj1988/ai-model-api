...
package com.github.aimodel.template;
...
/**
 * 使用StringTemplate（ST）v4库渲染模板。
 *
 * <p>
 * 此呈现器允许自定义分隔符、缺少模板变量时的验证行为，以及在验证期间如何处理StringTemplate的内置函数。
 * <p>
 * 使用 {@link #builder()} 创建和配置实例。
 * <p>
 * <b>线程安全：</b> 这个类可以安全地并发使用。每次调用
 * {@link #apply(String, Map)} 创建一个新的StringTemplate实例，线程之间不共享可变状态。
 *
    ...
    private Set<String> validate(ST st, Map<String, Object> templateVariables) {
        ...
        if (!missingVariables.isEmpty()) {
            if (this.validationMode == ValidationMode.WARN) {
                logger.warn(String.format(VALIDATION_MESSAGE, missingVariables));
            } else if (this.validationMode == ValidationMode.THROW) {
                throw new IllegalStateException(String.format(VALIDATION_MESSAGE, missingVariables));
            }
        }
