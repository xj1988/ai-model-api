...
import com.github.aimodel.model.ModelResponse;
...
public class ChatResponse implements ModelResponse<Generation> {
    ...
    public boolean hasFinishReasons(Set<String> finishReasons) {
        ...
        return this.generations.stream().anyMatch(generation -> {
            String finishReason = (generation.getMetadata().getFinishReason() != null)
                    ? generation.getMetadata().getFinishReason() : "";
            List<String> lowerCaseReasons = finishReasons.stream()
                    .map(String::toLowerCase)
                    .collect(Collectors.toList());
            return lowerCaseReasons.contains(finishReason.toLowerCase());
        });
    }
